...
import {
  isBoolean, isNumber, isArray, prop,
} from '../../uilt';

const checkModel = (obj, key, newValue) => {
  if (wmodelsMap.getWmodel('text')) {
    const currentmodelMap = wmodelsMap.getWmodel('text');
  ...
  if (wmodelsMap.getWmodel('checkbox')) {
    const currentmodelMap = wmodelsMap.getWmodel('checkbox');
    currentmodelMap.has(obj) && currentmodelMap.get(obj).get(key).forEach((input) => {
  ...
  if (wmodelsMap.getWmodel('radio')) {
    const currentmodelMap = wmodelsMap.getWmodel('radio');
    currentmodelMap.has(obj) && currentmodelMap.get(obj).get(key).forEach((input) => {
      /** 这里不用全等判断 因为需要 1 == “1” */
  ...
  if (wmodelsMap.getWmodel('select')) {
    const currentmodelMap = wmodelsMap.getWmodel('select');
    ...
    currentmodelMap.has(obj) && currentmodelMap.get(obj).get(key).forEach((select) => {
      [...select.options].forEach((option) => {
        ...
        if (isMultiple) {
          isSelected = newValue.findIndex(val => val == option.text) !== -1;
        } else {
