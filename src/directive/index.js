...
import wtext from './w-text';
import whtml from './w-html';
import wonce from './w-once';
import wbind from './w-bind';
import wshow from './w-show';
import won from './w-on';
import wif from './w-if';
import welse from './w-else';
import welseif from './w-else-if';
import wfor from './w-for';
import wmodel from './w-model';
...
const wModelHandle = function (data, modle, wue, InputType, e) {
  if (e.target === this) {
    ...
    if (InputType === 'checkbox') {
      const modleVal = getTemplateValue(wue.data, modle, modle);
      if (isBoolean(modleVal)) {
        const isChecked = !modleVal;
